...
static void ndpi_init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_str)
{
  ...
  ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_ACTIVISION,
                          ...
                          ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                          ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
  ndpi_set_proto_defaults(ndpi_str, 1 /* cleartext */, 1 /* app proto */, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_ERSPAN,
                          "ERSPAN", NDPI_PROTOCOL_CATEGORY_GAME,
                          ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                          ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
...
#define MATCH_DEBUG_INFO(fmt, ...)    \
  if (txt->option & AC_FEATURE_DEBUG) \
...
{
  if (((a & 0xFF000000) == 0x0A000000 /* 10.0.0.0/8 */) || ((a & 0xFFF00000) == 0xAC100000 /* 172.16.0.0/12 */) || ((a & 0xFFFF0000) == 0xC0A80000 /* 192.168.0.0/16 */) || ((a & 0xFF000000) == 0x7F000000 /* 127.0.0.0/8 */) || ((a & 0xF0000000) == 0xE0000000 /* 224.0.0.0/4 */))
    return (0);
  else
    return (1);
}
                                        ...
                                        u_int16_t port /* network byte order */)
{
  ...
  if (node)
  {
    ...
    {
      if ((node->value.u.uv16[i].additional_user_value == 0) || (node->value.u.uv16[i].additional_user_value == port))
...
void ndpi_exit_detection_module(struct ndpi_detection_module_struct *ndpi_str)
{
  if (ndpi_str != NULL)
  {
    ...
    if (ndpi_str->protocols)
      ndpi_ptree_destroy(ndpi_str->protocols);
    if (ndpi_str->ip_risk_mask)
      ndpi_ptree_destroy(ndpi_str->ip_risk_mask);
    if (ndpi_str->ip_risk)
    ...
    case NDPI_ICMP_PROTOCOL_TYPE:
      if (flow)
      {
        ...
        else
        {
          ...
          if (((icmp_type >= 44) && (icmp_type <= 252)) || (icmp_code > 15))
            ndpi_set_risk(flow, NDPI_MALFORMED_PACKET, NULL);
    ...
    case NDPI_ICMPV6_PROTOCOL_TYPE:
      if (flow)
      {
        ...
        else
        {
          ...
          if (((icmp6_type >= 5) && (icmp6_type <= 127)) || ((icmp6_code >= 156) && (icmp6_type != 255)))
            ndpi_set_risk(flow, NDPI_MALFORMED_PACKET, NULL);
        }
      }
...
int load_protocols_file_fd(struct ndpi_detection_module_struct *ndpi_str, FILE *fd)
{
  ...
  while (1)
  {
    ...
    while (((line = fgets(line, line_len, fd)) != NULL) && strlen(line) > 0 && (line[strlen(line) - 1] != '\n'))
    {
      i = strlen(line);
      old_buffer = buffer;
...
static int ndpi_callback_init(struct ndpi_detection_module_struct *ndpi_str)
{
  ...
  /* Erspan */
  init_erspan_dissector(ndpi_str, &a);
                                        ...
                                        const NDPI_PROTOCOL_BITMASK *dbm, int count_only)
{
  ...
  {
    if (!NDPI_ISSET(dbm, ndpi_str->callback_buffer[a].ndpi_protocol_id))
      continue;
    if (!ndpi_proto_cb_tcp_payload(ndpi_str, a))
      continue;
    if (!count_only)
    {
  ...
  for (a = 0; a < ndpi_str->callback_buffer_size; a++)
  {
    if (!NDPI_ISSET(dbm, ndpi_str->callback_buffer[a].ndpi_protocol_id))
      continue;
    if (!ndpi_proto_cb_tcp_nopayload(ndpi_str, a))
      continue;
    if (!count_only)
    {
      NDPI_LOG_DBG2(ndpi_str,
  ...
  for (a = 0; a < ndpi_str->callback_buffer_size; a++)
  {
    if (!NDPI_ISSET(dbm, ndpi_str->callback_buffer[a].ndpi_protocol_id))
      continue;
    if (!ndpi_proto_cb_udp(ndpi_str, a))
      continue;
    if (!count_only)
    {
  ...
  for (a = 0; a < ndpi_str->callback_buffer_size; a++)
  {
    if (!NDPI_ISSET(dbm, ndpi_str->callback_buffer[a].ndpi_protocol_id))
      continue;
    if (!ndpi_proto_cb_other(ndpi_str, a))
      continue;
    if (!count_only)
    {
...
static u_int8_t ndpi_is_multi_or_broadcast(struct ndpi_packet_struct *packet)
{
  ...
  if (packet->iph)
  {
    ...
    if (((daddr & 0xE0000000) == 0xE0000000 /* multicast */) || ((daddr & 0x000000FF) == 0x000000FF /* last byte is 0xFF, not super correct, but a good approximation */) || ((daddr & 0x000000FF) == 0x00000000 /* last byte is 0x00, not super correct, but a good approximation */) || (daddr == 0xFFFFFFFF))
      return (1);
  }
                                     ...
                                     struct ndpi_flow_struct *flow)
{
  ...
  if (ndpi_str->max_payload_track_len > 0 && packet->payload_packet_len > 0)
  {
    ...
    if (flow->flow_payload != NULL)
    {
      ...
      for (i = 0; (i < packet->payload_packet_len) && (flow->flow_payload_len < ndpi_str->max_payload_track_len); i++)
      {
  ...
  if (tcph != NULL)
  {
    ...
    if ((ndpi_str->input_info == NULL) || ndpi_str->input_info->seen_flow_beginning == NDPI_FLOW_BEGINNING_UNKNOWN)
    {
                                     ...
                                     ndpi_protocol *ret)
{
  ...
  case NDPI_PROTOCOL_TLS:
    ...
    */
    if ((flow->guessed_protocol_id_by_ip == NDPI_PROTOCOL_MICROSOFT_AZURE) && (ret->master_protocol == NDPI_PROTOCOL_UNKNOWN) && ndpi_str->msteams_cache)
    {
      u_int16_t dummy;
...
static void ndpi_check_tcp_flags(struct ndpi_flow_struct *flow)
{
  ...
  if ((flow->l4.tcp.cli2srv_tcp_flags & TH_SYN) && (flow->l4.tcp.srv2cli_tcp_flags & TH_RST) && (flow->packet_counter == 0 /* Ignore connections terminated by RST but that exchanged data (3WH + RST) */))
    ndpi_set_risk(flow, NDPI_TCP_ISSUES, "Connection refused (server)");
  else if ((flow->l4.tcp.cli2srv_tcp_flags & TH_SYN) && (flow->l4.tcp.cli2srv_tcp_flags & TH_RST) && (flow->packet_counter == 0 /* Ignore connections terminated by RST but that exchanged data (3WH + RST) */))
                                ...
                                ndpi_protocol_category_t category)
{
  ...
  return (ndpi_domain_classify_add(ndpi_str, ndpi_str->custom_categories.sc_hostnames_shadow,
                                   (u_int16_t)category, (char *)name_to_add)
              ? 0
              : -1);
#endif
}
                                                        ...
                                                        ndpi_protocol *returned_proto)
{
  ...
  */
  ...
  if (ndpi_is_ntop_protocol(returned_proto))
    return (1);
...
ret_protocols:
  ...
  if ((!flow->risk_checked) && ((ret.master_protocol != NDPI_PROTOCOL_UNKNOWN) || (ret.app_protocol != NDPI_PROTOCOL_UNKNOWN)))
  {
    ...
    if (found && (found->proto->protoId != NDPI_PROTOCOL_UNKNOWN) && (found->proto->protoId != ret.master_protocol) && (found->proto->protoId != ret.app_protocol))
    {
      ...
      if (!ndpi_check_protocol_port_mismatch_exceptions(found, &ret))
      {
        ...
        */
        ...
        if (!found)
        {
          ...
          if ((r == NULL) || ((r->proto->protoId != ret.app_protocol) && (r->proto->protoId != ret.master_protocol)))
          {
  ...
  if (!flow->tree_risk_checked)
  {
    ...
    if (ndpi_str->ip_risk && packet->iph && ndpi_is_public_ipv4(ntohl(packet->iph->saddr)) && ndpi_is_public_ipv4(ntohl(packet->iph->daddr)))
    {
      struct in_addr addr;
      ...
      net_risk = ndpi_network_risk_ptree_match(ndpi_str, &addr);
             ...
             packet->iphv6)
    { /* TODO: some checks on "local" addresses? */
      struct in6_addr addr;
  ...
  if ((ret.app_protocol == NDPI_PROTOCOL_UNKNOWN) && (packet->payload_packet_len > 0) && (flow->packet_counter <= 5))
  {
                          ...
                          u_int16_t proto_id)
{
  ...
  if ((proto_id >= ndpi_str->ndpi_num_supported_protocols) || (!ndpi_is_valid_protoId(proto_id)) || (ndpi_str->proto_defaults[proto_id].protoName == NULL))
                        ...
                        char *name, u_int8_t is_hostname, u_int8_t check_subproto)
{
  ...
  else
  {
    ...
    if ((!name) || (strchr(name, '_') != NULL) || (strchr(name, '-') != NULL) || (ends_with(ndpi_str, name, "in-addr.arpa")) || (ends_with(ndpi_str, name, "ip6.arpa"))
        /* Ignore TLD .local .lan and .home */
        || (ends_with(ndpi_str, name, ".local")) || (ends_with(ndpi_str, name, ".lan")) || (ends_with(ndpi_str, name, ".home")))
      return (0);
    ...
    if (len >= 5)
    {
      ...
      if (
          (is_hostname && (num_dots > 5) && (!first_element_is_numeric)) || (max_num_char_repetitions > 5 /* num or consecutive repeated chars */)
          /*
      ...
      if (num_bigram_checks
          ...
          && ((num_found == 0) || ((num_digits > 5) && (num_words <= 3) && (num_impossible > 0)) || enough(num_found, num_impossible) || ((num_trigram_checked > 2) && ((num_trigram_found < (num_trigram_checked / 2)) || ((num_trigram_vowels < (num_trigram_found - 1)) && (num_dash == 0) && (num_dots > 1) && (num_impossible > 0))))))
        rc = 1;
                            ...
                            int normalize)
{
  ...
  else
  {
    for (i = 0; i < len; i++)
    {
      ...
      if (!c)
        break;
      if (normalize & NDPI_HOSTNAME_NORM_LC)
        c = tolower(c);
